...
class IssueBloc extends Bloc<IssueEvent, IssueState> {
  ...
  Future<void> _onloadIssueInfo(OnLoadIssueInfo event, Emitter<IssueState> emit)async {
      try{
         emit(IssueState(requestStatus: RequestStatus.loading));
        final response=await getIssues.execute(
          NoParams());
         
         emit(IssueState(requestStatus: RequestStatus.success,responseIntegration: response));
      } catch(e){
            emit(IssueState(requestStatus: RequestStatus.error,errorMessage: e.toString())); 
      }
  }
}
